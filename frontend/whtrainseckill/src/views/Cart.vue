    ...
    <div class="cart-page">
        ...
        <el-table v-if="cartItems.length > 0" :data="cartItems" stripe>
            ...
            <el-table-column label="数量">
                <template #default="{ row }">
                    <el-input-number v-model="row.quantity" :min="1" :max="row.stock"
                        @change="updateCart(row.productId, row.quantity)"></el-input-number>
                </template>
            </el-table-column>
            ...
            <el-table-column label="操作" width="100">
                <template #default="{ row }">
                    <el-button type="danger" size="small" @click="removeFromCart(row.productId)">
                        删除
                    </el-button>
                </template>
            </el-table-column>
...
export default {
    ...
    methods: {
        ...
        updateCart(productId, quantity) {
            this.$axios.put(`/api/user/cart/update`, null, {
                params: {
                    productId: productId,
